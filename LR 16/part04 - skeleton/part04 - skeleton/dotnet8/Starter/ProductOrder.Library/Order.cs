...
public class OrderReader : DataReader
{
    ...
    public async Task<Order> GetOrderAsync(int orderId)
    {
        ...
        try
        {
            Task<Order> orderDetailsTask = GetOrderDetailsAsync(orderId);
            Task<Customer> customerTask = customerReader.GetCustomerForOrderAsync(orderId);
            Task<List<Product>> productTask = productReader.GetProductsForOrderAsync(orderId);
